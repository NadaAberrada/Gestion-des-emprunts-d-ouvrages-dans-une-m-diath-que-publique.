...
if (isset($_GET['id'])) {
    ...
    $delete = $conn->query("DELETE  FROM`ouvrage` WHERE `ouvre_id`='$id'");
}
?>
...
<head>
    ...
    <script>
        ...
        function confirmDelete(id) {
            ...
            let html = "";
            var mealList = document.getElementById("card");
            html += "<div class='modal fade text-center'  id='exampleModal'  tabindex='-1' aria-labelledby='exampleModalLabel' aria-hidden='true'>" +
                "<div class='modal-dialog'> <div class='modal-content'> <div class='modal-header'>" +
                "<h1 class='modal-title fs-5' id='exampleModalLabel'style='margin-left:32%'></h1>" +
                "<button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Close'></button></div>" +
                "<h4 class='modal-body'> do you really want to delete this ?</h4>" +

                "<div class='modal-footer'>" + "<button type='button' class='btn btn-secondary' data-bs-dismiss='modal'>Close</button>" +
                " <button type='button' class='btn btn-primary' id='confirmDeleteBtn'>Save changes</button>" +
                "</div></div></div></div>";

            mealList.insertAdjacentHTML("afterbegin", html);

            document.getElementById("confirmDeleteBtn").addEventListener("click", function() {
                window.location = "acueilAdmin.php?id=" + id;
...
<body>
            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                    <div class="navbar-nav ms-auto">
                        ...
                        <a class="nav-link navbar-text active" aria-current="page" href="./addEmprunt.php" style="color:black">Add Borrower</a>
                        <a class="nav-link navbar-text active" aria-current="page" href="./EditEmprunt.php" style="color:black">Edit Borrower</a>
                        <a class="nav-link navbar-text active" aria-current="page" href="./AddOuvrage.php" style="color:black">Add Work</a>
    ...
    <main>
        ...
        <div class="container" id="test">
            <div class="row justify-content-center mt-4" id="card"></div>
        </div>
        ...
        <div class="card" style="box-shadow: 6px 5px 17px rgba(0, 0, 0, 0.32);">
            ...
            <div class="card-body">
                ...
                <div class="d-flex  flex-wrap justify-content-between align-items-center" style="margin-top: 9%;">
                    <div class="btn-group">
                        <a onclick="confirmDelete(' . $ligne["ouvre_id"] . ')"id="Delete" data-bs-toggle="modal"
                         data-bs-target="#exampleModal" id="edit" class="btn btn-l btn-outline-secondary  col-md-2  col-6"
                         style="background-color: #f7e1c1; border-radius: 22px; color: black;" >Delete</a>
                    </div>  
                    <div class="btn-group">
                   
                          <a  href="EditOuvrage.php?id='.$ligne["ouvre_id"].'" class="btn btn-l btn-outline-secondary  col-md-2 col-6" style="background-color: #f7e1c1;
                           border-radius: 22px; color: black;">Edit</a>
                   
                      </div>
                    <div class="btn-group">
                    <a onclick="Detail(\'' . $ligne['ouvre_id'] . '\',\'' . $ligne['ouvre_titre'] . '\', \'' . $ligne['ouvre_etat'] . '\', \'' . $ligne['ouvre_auteur'] . '\', \'' . $ligne['ouvre_editionD'] . '\', \'' . $ligne['ouvre_achatD'] . '\', \'' . $ligne['ouvre_pages'] . '\', \'' . $ates . '\')" 
                    data-bs-toggle="modal" data-bs-target="#exampleModal' . $ligne['ouvre_id'] . '" 
                    id="edit" class="btn btn-l btn-outline-secondary  col-md-2 col-6" style="background-color: #f7e1c1; border-radius: 22px; color: black;" >Details</a>
                    </div>
                </div>
            </div>
        </div>
