...
<body>
    ...
    <div id="border">
        <form method="POST" action="" enctype="multipart/form-data" style="margin-left:5%;margin-right:5%">
            <h3 class="mb-5 text-center">Add Borrower</h3>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $idres = $_POST["idres"];
        $datelimit = date("Y-m-d", strtotime("+15 days"));
        $conn = new PDO("mysql:host=localhost;dbname=biblio;port=3306;charset=UTF8", 'root', '');
        // Prepare the SQL statement
        $stmt = $conn->prepare(" INSERT INTO `emprunt`( `empr_retour`, `bib_id`, `reserve_id`) VALUES (?, ?, ?)");

        $stmt->bindParam(1, $datelimit);
        $stmt->bindParam(2, $idadmin);
        $stmt->bindParam(3, $idres);
